...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			if category.name == "vip" then
				table.insert(options, ('%s: <span style="color:#17ACD1;">%s</span>'):format(vehicle.name, _U('precioatecoin', ESX.Math.GroupDigits(vehicle.price))))
			else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			align = 'bottom-right',
			ESX.ShowAdvancedNotification("Concesionario", "~b~"..vehicleData.name, ("Su precio es: ~y~"..ESX.Math.GroupDigits(vehicleData.price) .."€"), "CHAR_CARSITE", 2),
			elements = {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('esx_biATENASlling:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('esx_vehicleshop:setVehicleOwnedATENASPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBosATENASsMenu', 'cardealer', function(data2, menu2)
				menu2.close()
			end)
