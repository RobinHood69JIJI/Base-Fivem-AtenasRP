...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
	MySQL.Async.fetchAll('SELECT jail_time FROM users WHERE identifier = @identifier', {
		...
		if result[1] and result[1].jail_time > 0 then
			TriggerEvent('esx_jaATENASil:sendToJail', xPlayer.source, result[1].jail_time, true)
		end
	end)
end)
...
ESX.RegisterCommand('jail', 'admin', function(xPlayer, args, showError)
	TriggerEvent('esx_jaATENASil:sendToJail', args.playerId, args.time * 60)
...
RegisterNetEvent('esx_jaATENASil:sendToJail')
AddEventHandler('esx_jaATENASil:sendToJail', function(playerId, jailTime, quiet)
