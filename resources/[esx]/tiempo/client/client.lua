...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('tiempo:SyncMe', {time = true, weather = true})
end)

RegisterNetEvent('tiempo:PauseSync')
AddEventHandler('tiempo:PauseSync', function(boolean)
    if boolean then
        PauseSync = true
...
RegisterNetEvent('tiempo:ForceUpdate')
AddEventHandler('tiempo:ForceUpdate', function(data, instant, first_sync)
...
RegisterNetEvent('tiempo:SyncWeather')
AddEventHandler('tiempo:SyncWeather', function(data, instant, sync_checking)
    if not PauseSync then
        CheckSnowSync(data.weather)
        Weather = data.weather
        Blackout = data.blackout
...
RegisterNetEvent('tiempo:SyncTime')
AddEventHandler('tiempo:SyncTime', function(data, sync_checking)
    if not PauseSync then
...
RegisterNUICallback('instanttime', function(data)
    TriggerServerEvent('tiempo:ToggleInstantChange:Time', data.instanttime)
end)

RegisterNUICallback('instantweather', function(data)
    TriggerServerEvent('tiempo:ToggleInstantChange:Weather', data.instantweather)
end)

RegisterNUICallback('change', function(data)
    NUI_status = false
    local settings = data.values
    TriggerServerEvent('tiempo:ForceUpdate', {weather = settings.weather, hours = settings.time, dynamic = settings.dynamic, blackout = settings.blackout, freeze = settings.freeze, instanttime = settings.instanttime, instantweather = settings.instantweather}, nil)
    if data.savesettings then
        print('Saving Settings - please wait 3 seconds ...')
        Citizen.Wait(3000)
        TriggerServerEvent('tiempo:SaveSettings')
...
RegisterNetEvent('tiempo:OpenUI')
AddEventHandler('tiempo:OpenUI', function(values)
    Open_UI(values)
end)

function Open_UI(values)
    TriggerEvent('tiempo:ToggleNUIFocus')
...
RegisterNetEvent('tiempo:ToggleNUIFocus')
AddEventHandler('tiempo:ToggleNUIFocus', function()
    NUI_status = true
    while NUI_status do
        Wait(5)
...
function CheckIfSynced(data)
    ...
    if Weather ~= data.weather or Blackout ~= data.blackout or FreezeTime ~= data.freeze then
        TriggerEvent('tiempo:SyncWeather', json.decode(Callback()), true, true)
        print('WEATHER SYNC CHECK FAILED - RESYNCING NOW')
        result = false
    end
    if Hours ~= data.hours then
        TriggerEvent('tiempo:SyncTime', json.decode(Callback()), true)
...
function Callback(plate)
    CB_id = CB_id + 1
    TriggerServerEvent('tiempo:Callback', CB_id, plate)
    local timeout = 0 while CB[CB_id] == nil and timeout <= 50 do Citizen.Wait(0) timeout=timeout+1 end
    return CB[CB_id]
end

RegisterNetEvent('tiempo:Callback')
AddEventHandler('tiempo:Callback', function(id, result)
    CB[id] = result
    Wait(5000)
    CB[id] = nil
end)