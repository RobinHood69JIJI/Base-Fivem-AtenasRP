...
RegisterCommand(Config.Command, function(source)
    ...
    if PermissionsCheck(_source) then
        TriggerClientEvent('tiempo:OpenUI', _source, {weather = Weather, time = Hours, dynamic = Dynamic, blackout = Blackout, freeze = FreezeTime, instanttime = InstantTimeChange, instantweather = InstantWeatherChange})
    else
        Notification(_source, Config.Locales[Config.Language]['invalid_permissions'])
    end
end)

RegisterServerEvent('tiempo:SyncMe')
AddEventHandler('tiempo:SyncMe', function(instant)
    TriggerClientEvent('tiempo:ForceUpdate', source, {weather = Weather, hours = Hours, mins = Mins, dynamic = Dynamic, blackout = Blackout, freeze = FreezeTime, instanttime = InstantTimeChange, instantweather = InstantWeatherChange}, instant, true)
end)

RegisterServerEvent('tiempo:ForceUpdate')
AddEventHandler('tiempo:ForceUpdate', function(data, instant)
    local _source = source
    if instant == nil then
    ...
    if PermissionsCheck(_source) then
        ...
        TriggerClientEvent('tiempo:ForceUpdate', -1, {weather = Weather, hours = data.hours, mins = Mins, dynamic = Dynamic, blackout = Blackout, freeze = FreezeTime, instanttime = InstantTimeChange, instantweather = InstantWeatherChange}, instant)
    else
    ...
    while true do
        ...
        if not FreezeTime then
            ...
            if not HasTriggered then
                HasTriggered = true
                TimeCounter = 0
                TriggerClientEvent('tiempo:SyncTime', -1, {hours = Hours, mins = Mins}, false)
            end
        end
                ...
                if #Group >= TimesChanged then
                    ...
                    for k, v in pairs(Group) do
                        if LastWeatherTable[v] == nil then
                            ...
                            Weather = v
                            TriggerClientEvent('tiempo:SyncWeather', -1, {weather = Weather, blackout = Blackout}, InstantTimeChange, false)
...
RegisterServerEvent('tiempo:Callback')
AddEventHandler('tiempo:Callback', function(id)
    TriggerClientEvent('tiempo:Callback', source, id, json.encode({weather = Weather, hours = Hours, mins = Mins, dynamic = Dynamic, blackout = Blackout, freeze = FreezeTime, instanttime = InstantTimeChange, instantweather = InstantWeatherChange}))
end)

RegisterServerEvent('tiempo:ToggleInstantChange:Time')
AddEventHandler('tiempo:ToggleInstantChange:Time', function(boolean)
    InstantTimeChange = boolean
end)

RegisterServerEvent('tiempo:ToggleInstantChange:Weather')
AddEventHandler('tiempo:ToggleInstantChange:Weather', function(boolean)
    InstantWeatherChange = boolean
end)

RegisterServerEvent('tiempo:SaveSettings')
AddEventHandler('tiempo:SaveSettings', function()
    SaveSettngs()
end)
