...
AddEventHandler('esx:playerLoaded', function(playerData)
	...
	if Config.EnableHud then
		...
		for i=1, #playerData.accounts, 1 do
			...
			local accountTpl = '<div id="hud_account_' .. playerData.accounts[i].name .. '"><img width = 25 src="img/accounts/' .. playerData.accounts[i].name .. '.png"/>&nbsp;{{money}}</div>'
			ESX.UI.HUD.RegisterElement('account_' .. playerData.accounts[i].name, i-1, 0, accountTpl, { money = 0 })
			ESX.UI.HUD.UpdateElement('account_' .. playerData.accounts[i].name, { money = ESX.Math.GroupDigits(ESX.Math.Round(playerData.accounts[i].money,0)) })
