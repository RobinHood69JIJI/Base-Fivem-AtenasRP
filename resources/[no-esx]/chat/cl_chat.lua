...
RegisterNetEvent('_chat:messageEntATENASered')
...
AddEventHandler('__cfx_internal:serverPrint', function(msg)
    ...
    SendNUIMessage({
        type = 'ON_MESSAGE',
        message = {templateId = 'test', multiline = true, args = {msg}}
    })
end)
...
AddEventHandler('chat:clear',function(name) SendNUIMessage({type = 'ON_CLEAR'}) end)
...
RegisterNUICallback('chatResult', function(data, cb)
    ...
    if not data.canceled then
        ...
        if data.message:sub(1, 1) == '/' then
            ...
            TriggerServerEvent('_chat:messageEntATENASered', GetPlayerName(id),{r, g, b}, data.message)
        end
    end
...
RegisterKeyMapping('chat', 'Abrir el chat', 'keyboard', 't')
