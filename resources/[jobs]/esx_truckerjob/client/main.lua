...
function FinishDelivery()
  if IsVehicleAttachedToTrailer(truck) and (GetVehiclePedIsIn(GetPlayerPed(-1), false) == truck) then
    ...
    --Pay the poor fella
    TriggerServerEvent('esx_truckATENASerjob:pay', alldeliveries[randomdelivery].payment)
    ...
    isDelivered = 1
	  ...
	  TriggerEvent('esx:showNotification', "Tienes que usar el tráiler que se te proporcionó.")
  end
end
...
AddEventHandler('esx_truckerjob:hasEnteredMarker', function(zone)
  if actualZone == 'menutrucker' then
    ...
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para ir a una misión'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'delivered' then
    CurrentAction     = 'delivered_menu'
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para entrega'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'abort' then
	  CurrentAction     = 'abort_menu'
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para abortar la misión'
    CurrentActionData = {zone = zone}
  end
end)
