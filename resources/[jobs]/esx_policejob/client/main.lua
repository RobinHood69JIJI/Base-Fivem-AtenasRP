...
function setUniform(uniform, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		...
		if uniformObject then
			...
			if uniform == 'chaleco_uno' then
				SetPedArmour(playerPed, 100)
			end
			if uniform == 'chaleco_dos' then
				SetPedArmour(playerPed, 100)
			end
		else
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('separador1'), uniform = 'separador1'},
		{label = _U('uniforme_uno'), uniform = 'uniforme_uno'},
		{label = _U('uniforme_dos'), uniform = 'uniforme_dos'},
		{label = _U('separador2'), uniform = 'separador2'},
		{label = _U('uniforme_geo'), uniform = 'uniforme_geo'},
		{label = _U('uniforme_uotm'), uniform = 'uniforme_uotm'},
		{label = _U('uniforme_sma'), uniform = 'uniforme_sma'},
		{label = _U('separador3'), uniform = 'separador3'},
		{label = _U('chaleco_uno'), uniform = 'chaleco_uno'},
		{label = _U('chaleco_dos'), uniform = 'chaleco_dos'},
		{label = _U('separador4'), uniform = 'separador4'},
		{label = _U('cinturon_uno'), uniform = 'cinturon_uno'},
		{label = _U('cinturon_dos'), uniform = 'cinturon_dos'},
	}

	if ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'lieutenant' or ESX.PlayerData.job.grade_name == 'sergeant' then
		table.insert(elements, {label = _U('boina'), value = 'boina'})
	end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						ExecuteCommand("me Le coloca las esposas/Le quita las esposas")
						TriggerServerEvent('esx_policejob:haATENASndcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						ExecuteCommand("me Le escolta ")
						TriggerServerEvent('esx_policejob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						ExecuteCommand("me Abre la puerta del vehiculo y lo sube")
						TriggerServerEvent('esx_policejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						ExecuteCommand("me Abre la puerta del vehiculo y lo baja")
						TriggerServerEvent('esx_policejob:OutVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'fine' then
						ExecuteCommand("me Saca papel y boli y comienza a redactar las multas")
						OpenFineMenu(closestPlayer)
					elseif action == 'jail' then
						ExecuteCommand("do Se veria llegar a un agente penitenciario y llevarse al sujeto")
						JailPlayer(GetPlayerServerId(closestPlayer))
					elseif action == 'license' then
						ExecuteCommand("me Saca la tablet y revisa sus licencias")
						ShowPlayerLicense(closestPlayer)
					elseif action == 'unpaid_bills' then
						ExecuteCommand("me Saca la tablet y revisa las multas impagadas")
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction', {
				...
				elseif DoesEntityExist(vehicle) then
					if action == 'vehicle_infos' then
						ExecuteCommand("me Saca la tablet y revisa la informacion del vehiculo")
						local vehicleData = ESX.Game.GetVehicleProperties(vehicle)
						OpenVehicleInfosMenu(vehicleData)
					elseif action == 'hijack_vehicle' then
						ExecuteCommand("do Se veria llegar a una grua de color rojo granate y llevarse el vehiculo")
		...
		elseif data.current.value == 'alertas_policiales' then
            local elements = {
                {label = "[ALERTA] Nivel 1", value = 'green'},
                {label = "[ALERTA] Nivel 2", value = 'yellow'},
                {label = "[ALERTA] Nivel 3", value = 'orange'},
                {label = "[ALERTA] Nivel 4", value = 'red'},
                {label = "[ALERTA] Quitar Nivel", value = 'close'}
            }
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_biATENASlling:sendBill', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('esx_biATENASlling:sendBill', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function JailPlayer(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'jail_menu', {
		...
		if jailTime == nil then
			...
			TriggerServerEvent("esx_jaATENASil:sendToJail", player, jailTime * 60)
			menu.close()
		end
...
RegisterNetEvent('esx_policejob:haATENASndcuff')
AddEventHandler('esx_policejob:haATENASndcuff', function()
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBosATENASsMenu', 'police', function(data, menu)
						menu.close()
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
		ESX.TriggerServerCallback('esx_society:getOnlATENASinePlayers', function(players)
