...
RegisterServerEvent('esx_tequilala:confiscatePlayerItem')
...
RegisterServerEvent('esx_tequilala:handcuff')
AddEventHandler('esx_tequilala:handcuff', function(target)
	...
	if xPlayer.job.name == 'tequilala' then
		...
		print(('esx_tequilala: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_tequilala:drag')
AddEventHandler('esx_tequilala:drag', function(target)
	...
	if xPlayer.job.name == 'tequilala' then
		...
		print(('esx_tequilala: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_tequilala:tequilalanVehicle')
AddEventHandler('esx_tequilala:tequilalanVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'tequilala' then
		TriggerClientEvent('esx_tequilala:tequilalanVehicle', target)
	else
		print(('esx_tequilala: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_tequilala:OutVehicle')
AddEventHandler('esx_tequilala:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'tequilala' then
		...
		print(('esx_tequilala: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_tequilala:getStockItem')
...
RegisterServerEvent('esx_tequilala:putStockItems')
...
ESX.RegisterServerCallback('esx_tequilala:getOtherPlayerData', function(source, cb, target)
...
ESX.RegisterServerCallback('esx_tequilala:getFineList', function(source, cb, category)
...
ESX.RegisterServerCallback('esx_tequilala:getVehicleInfos', function(source, cb, plate)
...
ESX.RegisterServerCallback('esx_tequilala:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
ESX.RegisterServerCallback('esx_tequilala:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_tequilala: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_tequilala: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'tequilala' then
			Citizen.Wait(5000)
...
RegisterServerEvent('esx_tequilala:spawned')
...
RegisterServerEvent('esx_tequilala:forceBlip')
...
RegisterServerEvent('esx_tequilala:message')
AddEventHandler('esx_tequilala:message', function(target, msg)
	TriggerClientEvent('esx:showNotification', target, msg)
end)

RegisterServerEvent('esx_tequilala:requestarrest')
AddEventHandler('esx_tequilala:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_tequilala:getarrested', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_tequilala:doarrested', _source)
end)

RegisterServerEvent('esx_tequilala:requestrelease')
AddEventHandler('esx_tequilala:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_tequilala:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_tequilala:douncuffing', _source)
end)
