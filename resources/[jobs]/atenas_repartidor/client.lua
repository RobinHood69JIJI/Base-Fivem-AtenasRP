...
local pizzeria = { x = 275.61, y = -961.33, z = 28.36}
local spawnfaggio = { x = 275.61, y = -961.33, z = 28.36, h = 25.01}
...
local casas = {
	...
	[2] = {name = "Vinewood Hills",   x = -614.68, y = 491.62 , z = 107.68},
	[3] = {name = "Rockford Hills",   x = -1051.85, y = 431.66 , z = 76.06 },
	[4] = {name = "Vespucci Canals",  x = -813.34 , y = -981.6 , z = 14.14 },
	...
	[10] = {name = "Rancho",           x = 398.75  , y = -1789.07, z = 29.15},
	[11] = {name = "Morningwood",      x = -1532.0  , y = -276.43, z = 48.54},
	[12] = {name ="Chamberlain Hills", x = -213.00, y =-1617.35 , z =37.35},
	[13] = {name ="Burton",            x = -84.54, y =-280.99 ,z = 44.55}
}

local isInJobPizz = false
local sigcasa = 0
local plateab = "ATEJOBS"
local isToHouse = false
local isToPizzaria = false
local multiplicador_De_dinero = 1.0 
local paga = 0
	...
	while true do
		...
		if isInJobPizz == false then
				...
				if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
					drawTxt("Pulsa E para iniciar la entrega",2, 1, 0.45, 0.03, 0.80,255,255,51,255)
					if IsControlJustPressed(1,38) then
						if ESX.PlayerData.job.name == "mcdonald" then
							isInJobPizz = true
							isToHouse = true
							sigcasa = math.random(1, 13)
							-- [INFO] TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0}, sigcasa)
							px = casas[sigcasa].x
							py = casas[sigcasa].y
							pz = casas[sigcasa].z
							distancia = round(GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, px,py,pz))
							paga = distancia * multiplicador_De_dinero
							-- [INFO] TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0}, distancia)
							spawn_faggio()
							Iracasa(casas,sigcasa)
						else
							ESX.ShowNotification('No perteneces a este trabajo. ¡Fuera de aquí!')
						end
					end
				end
		end
		if isToHouse == true then
			...
			drawTxt("Ve a "..destinol .." y entrega el pedido",4, 1, 0.45, 0.92, 0.70,255,255,255,255)
			...
			if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
				drawTxt("Pulsa E para entregar el pedido",2, 1, 0.45, 0.03, 0.80,255,255,51,255)
				if IsControlJustPressed(1,38) then
					posibilidad = math.random(1, 100)
					if (posibilidad > 70) and (posibilidad < 90) then
						propina = math.random(1500, 2000)
						TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0},"Toma de propina : "..propina.."€")
		...
		if isToPizzaria == true then
			drawTxt("Vuelve al McDonald´s para recoger tu dinero",4, 1, 0.45, 0.92, 0.70,255,255,255,255)
			DrawMarker(1,pizzeria.x,pizzeria.y,pizzeria.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,255,51, 200, 0, 0, 0, 0)
				if GetDistanceBetweenCoords(pizzeria.x,pizzeria.y,pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
					...
					if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
						if IsControlJustPressed(1,38) then
							if IsInVehicle() then
								TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0},"Gracias por hacer la entrega, toma tu paga: "..paga.."€")
								...
								TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0},"No te pagaré si no me das mi moto, lo siento.")
							end
						end
					else
						TriggerEvent('chatMessage', 'Jefe de McDonald´s', {255, 0, 0},"No te pagaré si no me das mi moto, lo siento.")
...
function spawn_faggio()
	...
	local spawned_car = CreateVehicle(vehicle, spawnfaggio.x,spawnfaggio.y,spawnfaggio.z, 346.0, - -869.73, 29.29, true, false)
